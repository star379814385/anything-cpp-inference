...
//     preprocess(input.img, img_process, transParam);
//     cv::Mat blob = cv::dnn::blobFromImage(img_process, 1.0, {img_process.cols, img_process.rows}, cv::Scalar(0, 0, 0), false, false, CV_32F);
//     // to onnx
//     std::vector<int64_t> YOLO_input_node_dims = {1, m_channel, img_process.rows, img_process.cols};
// 	Ort::Value input_tensor = Ort::Value::CreateTensor<float>(Ort::MemoryInfo::CreateCpu(OrtDeviceAllocator, OrtMemTypeCPU), (float*)blob.data, 3 * img_process.cols * img_process.rows, YOLO_input_node_dims.data(), YOLO_input_node_dims.size());
//     // Ort::Value input_tensor = Ort::Value::CreateTensor<float>(Ort::MemoryInfo::CreateCpu(OrtDeviceAllocator, OrtMemTypeCPU), blob.data, 3 * input_h * input_w, YOLO_input_node_dims.data(), YOLO_input_node_dims.size());
...
//     postprocess(output_tensor, output, transParam);
...
bool ONNXInference::preprocess(const cv::Mat &src, cv::Mat &dst, TransformParam &transformParam)
{
    ...
    if(m_swapRb) cv::cvtColor(dst, dst, cv::COLOR_BGR2RGB);
